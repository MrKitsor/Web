...
abstract class TableModule
{
    ...
    public function deleteImage($id)
    {
        ...
        if (file_exists($_SERVER['DOCUMENT_ROOT'].'/API/inc/images/'.$remove_img_result['img_path'])) {
            unlink($_SERVER['DOCUMENT_ROOT'] . '/API/inc/images/' . $remove_img_result['img_path']);
        }
    }
	...
	public function read($needcrews = false,$crewfilter='default')
	{
        ...
        if ($needcrews === true)
        {
            ...
            // $sql = "SELECT students.*, groups.groupName, groups.speciality FROM students LEFT JOIN groups ON students.idGroup = groups.id; ";
            $sql = "SELECT types.*, categories.category, categories.description FROM types LEFT JOIN categories ON types.category = categories.category; ";
        }
