...
export default {
  ...
  actions: {
    ...
    addItem: async ({ commit }, { category, description }) => {
      const item = await api.add({ category, description });
      commit('addItem', item);
    },
    updateItem: async ({ commit }, { id, category, description }) => {
      const item = await api.update({ id, category, description });
